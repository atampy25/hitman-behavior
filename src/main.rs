	...
	match std::env::args().nth(1).as_deref() {
		...
		Some("h2") => match std::env::args().nth(2).as_deref() {
			Some("convert-json") => {
				...
				let tree = h2::BehaviorTree::from_raw(raw).map_err(|e| format!("{e}")).unwrap();
			...
			Some("generate-json") => {
				...
				let tree: h2::BehaviorTree = serde_json::from_slice(&fs::read(input_path).unwrap()).unwrap();
			...
			Some("generate-txt") => {
				...
				let tree = h2::BehaviorTree::from_pseudocode(&fs::read_to_string(input_path).unwrap()).unwrap();
		...
		_ => panic!("1st argument must be h1, h2, or h3")
	}
}
